...
export default function NewBlogPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success('Blog yazısı yaradıldı')
        router.push('/admin/blogs')
      } else {
        const error = await response.json()
        toast.error(error.error || 'Blog yazısını yaratmaq mümkün olmadı')
      }
    } catch (error) {
      console.error('Error creating blog:', error)
      toast.error('Blog yazısını yaratmaq mümkün olmadı')
    } finally {
      setLoading(false)
    }
  }
